...
void main() async {
  ...
  try {
    ...
    runApp(const KuasirApp(useFirebase: true));
  } catch (e) {
    print('Firebase initialization failed: $e');
    useFirebase = false;
    // If Firebase fails, show error but allow to continue with local data
    runApp(MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          title: const Text('Kuasir - Error'),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text(
                  'Aplikasi akan berjalan dengan penyimpanan lokal saja.\nData tidak akan tersinkron ke cloud.',
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    runApp(const KuasirApp(useFirebase: false));
                  },
                  ...
                  child: const Text('Lanjutkan Tanpa Cloud'),
                ),
                const SizedBox(height: 16),
                ElevatedButton(
                  onPressed: () => main(),
...
class KuasirApp extends StatelessWidget {
  final bool useFirebase;
  
  const KuasirApp({super.key, this.useFirebase = true});

  @override
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ChangeNotifierProvider(create: (_) => NoAuthBarangProvider(useFirebase: useFirebase)),
...
class _LoadingScreenState extends State<LoadingScreen> {
  ...
  Future<void> _initializeApp() async {
    ...
    if (mounted) {
      ...
      if (widget.useFirebase) {
        try {
          await Provider.of<NoAuthBarangProvider>(context, listen: false).loadBarang();
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (context) => NoAuthDashboardScreen(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          child: ListTile(
                            ...
                            subtitle: Text(_formatCurrency(cartItem.barang.harga)),
                            trailing: Row(
              ...
              children: [
                ...
                ...cartProvider.cartItems.map((item) => Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Expanded(
                        child: Text('${item.barang.namaBarang} x${item.quantity}'),
                      ),
  ...
  Future<void> _processCheckout(String notes) async {
    ...
    final barangProvider = Provider.of<NoAuthBarangProvider>(context, listen: false);
  ...
  void _showTransaksiDetail(Transaksi transaksi) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Detail Transaksi #${transaksi.id?.substring(0, 8) ?? 'Unknown'}'),
        content: SizedBox(
          width: double.maxFinite,
          height: 400,
          child: SingleChildScrollView(
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('${_formatCurrency(item.harga)} x ${item.quantity}'),
                            ],
                          ),
                        ),
                        Text(
...
class NoAuthDashboardScreen extends StatefulWidget {
  final bool useFirebase;
  
  const NoAuthDashboardScreen({super.key, required this.useFirebase});

  @override
  State<NoAuthDashboardScreen> createState() => _NoAuthDashboardScreenState();
}

class _NoAuthDashboardScreenState extends State<NoAuthDashboardScreen> {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('${barang.kategori} • Rp ${barang.harga.toStringAsFixed(0)}'),
                        Text('Stok: ${barang.stok}', style: TextStyle(
                          color: barang.stok > 10 ? Colors.green : Colors.orange,
                          fontWeight: FontWeight.w600,
                        )),
                      ],
                    ),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Consumer<CartProvider>(
                      builder: (context, cartProvider, child) {
                        ...
                        final cartQuantity = cartProvider.getItemQuantity(barang.id!);
                            ...
                            subtitle: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Stok: ${barang.stok}',
                                  style: TextStyle(
                                    color: barang.stok <= 5 ? Colors.red : Colors.green,
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              Text(
                                _formatDateTime(transaksi.tanggalTransaksi),
                              ),
                              Text(
            ...
            onPressed: () async {
              ...
              try {
                final barang = Barang(
                  ...
                  kodeBarcode: barcodeController.text.isEmpty ? null : barcodeController.text,
                  createdAt: DateTime.now(),
                  updatedAt: DateTime.now(),
                );

                await Provider.of<NoAuthBarangProvider>(context, listen: false)
                    .addBarang(barang);
  ...
  void _showEditBarangDialog(Barang barang) {
    ...
    final priceController = TextEditingController(text: barang.harga.toString());
    final stockController = TextEditingController(text: barang.stok.toString());
    final barcodeController = TextEditingController(text: barang.kodeBarcode ?? '');
